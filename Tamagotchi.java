...
class Tamagotchi {
    ...
    // Método para verificar se o Tamagotchi está doente
    ...
    // Método para dar remédio (opção 4)
    public void darRemedioOpcao4() {
        if (doente) { // Se estiver doente, cura
            doente = false; 
            System.out.println(nome + " tomou remédio e foi curado!");
        } else { // Se não estiver doente, não faz nada
            System.out.println(nome + " não está doente. Não é necessário dar remédio.");
        }
    }
